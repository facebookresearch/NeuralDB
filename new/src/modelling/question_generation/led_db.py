...
def main():
    def process_data_to_model_inputs(batch):
        ...
        new_batch = dict()
        new_batch["input_ids"] = inputs.input_ids
        new_batch["attention_mask"] = inputs.attention_mask

        # create 0 global_attention_mask lists
        new_batch["global_attention_mask"] = len(new_batch["input_ids"]) * [
            [0 for _ in range(len(new_batch["input_ids"][0]))]
        ]

        # since above lists are references, the following line changes the 0 index for all samples
        new_batch["global_attention_mask"][0][0] = 1
        new_batch["labels"] = outputs.input_ids

        # We have to make sure that the PAD token is ignored
        new_batch["labels"] = [
            [-100 if token == tokenizer.pad_token_id else token for token in labels]
            for labels in new_batch["labels"]
        ]

        return {k:torch.LongTensor(v) for k,v in new_batch.items()}
    ...
    trainer = Seq2SeqTrainer(
        ...
        data_collator=process_data_to_model_inputs, #train_dataset.collate_fn, # Seq2SeqDataCollator(tokenizer, data_args, training_args.tpu_num_cores),
