...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"resources/v1.6_25_ds_sg_{args.top}"):
        os.makedirs(f"resources/v1.6_25_ds_sg_{args.top}",exist_ok=True)
    with open(f"resources/v1.6_25/{args.split}.jsonl") as f, open(
            f"resources/v1.6_25_ds_sg_{args.top}/{args.split}.jsonl", "w+"
        ) as outf:
        ...
        for idx, line in enumerate(f):
            ...
            for query, *predictions in zip(db["queries"], *files):
                ...
                precisions.append(precision(query["facts"], [h[0] for h in high]))
                recalls.append(recall(query["facts"], [h[0] for h in high]))
