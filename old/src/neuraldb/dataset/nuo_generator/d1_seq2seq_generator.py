...
class D1Seq2SeqSpecificGenerator(InstanceGenerator):
    def __init__(
        self,
    ...
    def _generate(self, instances):
        for idx, instance in enumerate(instances):
            if "ssid_id" in instance:
                if instance["ssid_id"] is not None:
                    facts = [
                        a[1]
                        for a in instance["instance"]["ssg_output"][instance["ssid_id"]]
                    ]
                else:
                    facts = []
                    for ssg in instance["instance"]["ssg_output"]:
                        for _, fact in ssg:
                ...
                facts = []
            if instance["outputs"] is None:
                instance["outputs"] = [None]
            query_tokens = self._tokenizer.tokenize(instance["instance"]["question"])
            fact_tokens = self._tokenizer.tokenize(
                " [SEP] ".join(facts) if len(facts) else "[EMPTY]"
            )
            projection_preprocessed = " [SEP] ".join(
                self._preprocess_answer(a) for a in instance["outputs"]
            )
    ...
    def _preprocess_answer(self, answer):
