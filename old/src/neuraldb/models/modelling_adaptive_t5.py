...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        # if self.is_decoder and retriever_bias is not None:
        #    scores += retriever_bias.unsqueeze(1).unsqueeze(1).expand_as(scores)
        ...
        if self.is_decoder and retriever_bias is not None:
            weights = weights * retriever_bias.unsqueeze(1).unsqueeze(1).expand_as(
                scores
            )
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (AdaptiveT5Model, AdaptiveT5ForConditionalGeneration)):
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, max_ret_layer=None):
        super().__init__(config)
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.max_ret_layer = max_ret_layer
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value_state) in enumerate(
            ...
            layer_outputs = layer_module(
                hidden_states,
                retriever_bias=retriever_bias,  # if self.is_decoder and i  == 0 else None, #>= len(self.block)-1 else None,
...
class AdaptiveT5Model(T5PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class AdaptiveT5ForConditionalGeneration(T5PreTrainedModel):
    def __init__(self, config, max_ret_layer):
        super().__init__(config)
        ...
        encoder_config = copy.deepcopy(config)
        self.encoder = T5Stack(encoder_config, self.shared, max_ret_layer=max_ret_layer)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        self.decoder = T5Stack(decoder_config, self.shared, max_ret_layer=max_ret_layer)
    ...
    def forward(
        ...
        # Decode
        assert retriever_bias is not None
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            inputs_embeds=decoder_inputs_embeds,
    ...
    def prepare_inputs_for_generation(
        self, input_ids, past, attention_mask, use_cache, retriever_bias, **kwargs
    ):
