...
class T5Attention(nn.Module):
    ...
    def forward(
        self, input, mask=None, kv=None, position_bias=None, cache=None, head_mask=None
    ):
        """
    ...
    def custom(self, q):
        def custom_forward(*inputs):
            ...
            scores += position_bias_mask
            weights = F.softmax(scores, dim=-1).type_as(
                scores
            )  # (bs, n_heads, qlen, klen)
            return weights
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        self, hidden_states, attention_mask=None, position_bias=None, head_mask=None
    ):
        norm_x = self.layer_norm(hidden_states)
            ...
            if i == 0:
                ...
                if self.is_decoder:
                    encoder_decoder_position_bias = layer_outputs[
                        4 if self.output_attentions else 2
                    ]
...
class T5MergeForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        """
        if encoder_outputs is None:
            hidden_states, attention_mask, context_ids = self.apply_encoder_list(
    ...
    def apply_encoder_list(self, input_ids, input_masks):
        ...
        concat_outputs = [
            torch.cat(
                (o, torch.zeros((1, max_length - o.size(1), self.model_dim)).cuda()),
                dim=1,
            )
        ...
        concat_masks = [
            torch.cat(
                (
                    ...
                    torch.zeros((1, max_length - sizes[i]), dtype=torch.bool).cuda(),
                ),
                dim=1,
            )
            for i in range(len(concat_outputs))
        ]
        concat_ids = [
            torch.cat(
                (ids, torch.zeros((1, max_length - ids.size(1))).long().cuda()), dim=1
            )
            for ids in concat_ids
        ]
    ...
    def generate(
        ...
        context_mask: Optional[Iterable[int]] = None,
        **model_specific_kwargs,
        ...
        if self.config.is_encoder_decoder:
            ...
            # Encode context
            encoder_outputs, attention_mask, context_ids = self.apply_encoder_list(
                context_ids, context_mask
            )
            encoder_outputs = (encoder_outputs,)
            context_outputs = None
        ...
        model_specific_kwargs.update(
            dict(
                context_ids=context_ids, context_mask=context_mask, context_outputs=None
            )
        )
