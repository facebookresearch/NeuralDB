...
def ndb_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training):
    features = []
    if is_training:
    ...
    for span in spans:
        ...
        end_position = 0
        if is_training:
            # For commands, if our document chunk does not contain an annotation
...
def ndb_convert_example_to_features_init(tokenizer_for_convert):
    global tokenizer
...
def ndb_convert_examples_to_features(
    ...
    with Pool(threads, initializer=ndb_convert_example_to_features_init, initargs=(tokenizer,)) as p:
        annotate_ = partial(
            ndb_convert_example_to_features,
    ...
    def _create_examples(self, input_data, set_type):
        ...
        for (i, line) in enumerate(input_data):
            ...
            example = NDBExample(
                ...
                answer_text=answer,
                start_position_character=context.find(answer)
            )
...
class NDBExample(object):
    """
    A single commands/test example for the Squad dataset, as loaded from disk.
